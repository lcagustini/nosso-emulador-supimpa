...
mirror:
    ...
    if (!internal_read) ppu.ram.buffer = readPPUByte(0x2000 + (addrs % 0x1000), true);
    addrs = (addrs - 0x3F00) % 0x20;
    if (addrs == 0x10 || addrs == 0x14 || addrs == 0x18 || addrs == 0x1C) addrs -= 0x10;
...
uint8_t readCPUByte(uint16_t addrs, bool internal_read) {
  ...
  if (addrs == 0x2007) {
    ...
    if (!internal_read) {
      ...
      ppu.scroll.temp_y += GET_VRAM_PPU_INCREMENT() ? 1 : 0;
      ppu.scroll.temp_x += GET_VRAM_PPU_INCREMENT() ? 0 : 1;
    }
    return data;
  }
